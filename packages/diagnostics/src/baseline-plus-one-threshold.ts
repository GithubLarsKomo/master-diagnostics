import { interpolateX } from './linear-interpolation';
import type { DiagnosticPoint, ThresholdEstimate } from './types';

const ALGORITHM = 'baseline-plus-one-mmol';
const VERSION = '1.0.0';

function usablePoints(points: readonly DiagnosticPoint[]): DiagnosticPoint[] {
  const usable = points
    .filter((point) => point.included)
    .filter((point) => point.lactateQualifier === undefined || point.lactateQualifier === 'EXACT')
    .map((point) => ({ ...point }))
    .sort((left, right) => left.watts - right.watts);

  if (usable.length < 2) {
    throw new Error('At least two included exact lactate points are required.');
  }
  for (const point of usable) {
    if (!Number.isFinite(point.watts) || !Number.isFinite(point.lactate)) {
      throw new TypeError('Diagnostic points must contain finite watts and lactate values.');
    }
  }
  for (let index = 1; index < usable.length; index += 1) {
    if (usable[index - 1]!.watts === usable[index]!.watts) {
      throw new Error('Included diagnostic points must have distinct watt values.');
    }
  }

  return usable;
}

export function calculateBaselinePlusOneThreshold(
  points: readonly DiagnosticPoint[],
  baselineLactate: number,
): ThresholdEstimate {
  if (!Number.isFinite(baselineLactate) || baselineLactate < 0) {
    throw new TypeError('Baseline lactate must be a finite non-negative number.');
  }

  const targetLactate = baselineLactate + 1;
  const usable = usablePoints(points);
  const exact = usable.filter((point) => point.lactate === targetLactate);

  if (exact.length > 1) {
    throw new Error(`Multiple included points equal ${targetLactate} mmol/L.`);
  }
  if (exact.length === 1) {
    const point = exact[0]!;
    return {
      watts: point.watts,
      lactate: targetLactate,
      ...(point.heartRate === undefined ? {} : { heartRate: point.heartRate }),
      algorithm: ALGORITHM,
      version: VERSION,
      warnings: [],
    };
  }

  const brackets: Array<[DiagnosticPoint, DiagnosticPoint]> = [];
  for (let index = 1; index < usable.length; index += 1) {
    const left = usable[index - 1]!;
    const right = usable[index]!;
    if (left.lactate < targetLactate && right.lactate > targetLactate) {
      brackets.push([left, right]);
    }
  }

  if (brackets.length === 0) {
    throw new Error(`No included exact points bracket baseline + 1 mmol/L (${targetLactate} mmol/L).`);
  }
  if (brackets.length > 1) {
    throw new Error(`Multiple intervals bracket baseline + 1 mmol/L (${targetLactate} mmol/L).`);
  }

  const [left, right] = brackets[0]!;
  const watts = interpolateX(
    left.watts,
    left.lactate,
    right.watts,
    right.lactate,
    targetLactate,
  );
  const heartRate = (
    left.heartRate === undefined || right.heartRate === undefined
      ? undefined
      : interpolateX(
        left.heartRate,
        left.lactate,
        right.heartRate,
        right.lactate,
        targetLactate,
      )
  );

  return {
    watts,
    lactate: targetLactate,
    ...(heartRate === undefined ? {} : { heartRate }),
    algorithm: ALGORITHM,
    version: VERSION,
    warnings: [],
  };
}
