...
describe('restore privacy effect journal', () => {
  it('persists independent immutable pending and committed markers with private permissions', async () => {
    const { targetDir, keyFile } = await fixture();
    const pendingResult = await persistSignedRestorePrivacyEffectRecord({
      targetDir, keyFile, record: pending(),
    });
    const committedResult = await persistSignedRestorePrivacyEffectRecord({
      targetDir, keyFile, record: committed(),
    });

    expect(pendingResult.created).toBe(true);
    expect(committedResult.created).toBe(true);
    expect(pendingResult.path).not.toBe(committedResult.path);
  ...
  it('accepts byte-identical retries but rejects conflicting content for the same execution phase', async () => {
    const { targetDir, keyFile } = await fixture();
    ...
    await expect(persistSignedRestorePrivacyEffectRecord({
      targetDir,
      keyFile,
      record: pending('2026-08-07T20:00:03.000Z'),
  ...
  it('rejects tampering and file-name/record mismatches', async () => {
    ...
    const committedName = restorePrivacyEffectFileName(committed());
    const renamedPath = join(targetDir, committedName);
