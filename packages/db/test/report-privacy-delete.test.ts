...
async function createTestDatabase(): Promise<Database> {
  const databasePath = `/tmp/masters-report-privacy-delete-${crypto.randomUUID()}.db`;
...
async function seed(db: Database) {
  ...
  await db.insert(schema.interpretations).values({
    ...
    releasedAt: '2020-01-01T12:00:00.000Z', releasedByUserId: 'trainer-a',
    createdAt: '2020-01-01T12:00:00.000Z', updatedAt: '2020-01-01T12:00:00.000Z',
  });
  await db.insert(schema.reportVersions).values({
    ...
    createdAt: '2020-01-01T13:00:00.000Z', updatedAt: '2020-01-01T13:00:00.000Z',
  });
...
describe('report version privacy deletion gate', () => {
  it('keeps normal report history immutable until the bound execution is ARTIFACTS_STAGED', async () => {
    ...
    await expect(db.delete(schema.reportVersions)
      .where(eq(schema.reportVersions.id, 'report-a'))).rejects.toThrow();
    ...
    expect(await db.select().from(schema.reportVersions)
      .where(eq(schema.reportVersions.id, 'report-a'))).toEqual([]);
  });

  it('does not authorize a report reference that was not captured by the execution manifest', async () => {
    const db = await createTestDatabase();
    await seed(db);
    const approval = await approveAthleteAnonymization(
      db, 'tenant-a', 'athlete-a', actor, capabilities, '2026-08-05T13:00:00.000Z',
    ...
    await db.insert(schema.reportVersions).values({
      id: 'report-late', tenantId: 'tenant-a', testId: 'test-a', interpretationId: 'interpretation-a',
      versionNumber: 2, locale: 'de', contentHash: `sha256:${'d'.repeat(64)}`,
      storageReference: 'tenant-a/test-a/de/report-late.pdf',
      createdAt: '2026-08-05T13:05:30.000Z', updatedAt: '2026-08-05T13:05:30.000Z',
    });
    await db.update(schema.athleteAnonymizationExecutions).set({
      status: 'ARTIFACTS_STAGED', artifactsStagedAt: '2026-08-05T13:06:00.000Z',
      updatedAt: '2026-08-05T13:06:00.000Z',
    ...
    await expect(db.delete(schema.reportVersions)
      .where(eq(schema.reportVersions.id, 'report-late'))).rejects.toThrow();
    expect(await db.select().from(schema.reportVersions)
      .where(eq(schema.reportVersions.id, 'report-late'))).toHaveLength(1);
  });
});
