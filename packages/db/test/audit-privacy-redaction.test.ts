...
async function seedReadyAthlete(db: Database, options: {
  ...
  await db.insert(schema.athletes).values({
    id: athleteId, tenantId: 'tenant-a', linkedUserId, firstName: 'Petra', lastName: 'Muster',
  ...
  await db.insert(schema.tests).values({
    id: 'test-a', tenantId: 'tenant-a', athleteId, deviceType: 'ROWERG', status: 'RELEASED',
    conductingTrainerUserId: 'trainer-a', startedAt: '2020-01-01T10:00:00.000Z',
    endedAt: testEndedAt, releasedAt: testEndedAt, currentVersion: 3,
    createdAt: '2020-01-01T09:00:00.000Z', updatedAt: testEndedAt,
  });
  await db.insert(schema.athleteDeletionRequests).values({
    id: 'deletion-a', tenantId: 'tenant-a', athleteId, status: 'COMPLETED',
    reason: 'Betroffenenrecht', requestedAt: '2025-01-01T00:00:00.000Z',
    decidedAt: '2025-01-02T00:00:00.000Z', decisionReason: 'Freigegeben',
    completedAt: deletedAt, createdAt: '2025-01-01T00:00:00.000Z', updatedAt: deletedAt,
  });
...
describe('controlled historical audit privacy redaction', () => {
  it('allows only the inventory-selected one-way redaction and keeps the proof immutable', async () => {
    const db = await createTestDatabase();
    const { athleteId, linkedUserId } = await seedReadyAthlete(db, {});
    const event = await appendAuditEvent(db, {
      tenantId: 'tenant-a', occurredAt: '2021-01-01T00:00:00.000Z',
      actorUserId: linkedUserId, actorRole: 'ATHLETE', authProvider: 'BETTER_AUTH',
      sessionId: 'athlete-session-a', action: 'athlete.updated', entityType: 'athlete',
      entityId: athleteId, source: 'WEB', reason: 'Petra Muster requested correction',
      before: { firstName: 'Petra', lastName: 'Muster', athleteId },
      after: { firstName: 'Petra-Maria', lastName: 'Muster', athleteId },
    });

    const result = await redactHistoricalAuditEventForAthlete(db, 'tenant-a', athleteId, adminActor, {
    ...
    expect(result.redactedFields).toEqual([
      'actorUserId', 'sessionId', 'reason', 'beforeJson', 'afterJson',
    ]);
    expect(result.matchedIdentifierClasses).toEqual(['ATHLETE_LINKED_USER', 'ATHLETE_NAME']);
    expect(result.event).toMatchObject({
      id: event.id, actorUserId: null, sessionId: null, action: 'athlete.updated',
      entityType: 'athlete', entityId: athleteId, reason: AUDIT_PRIVACY_REDACTED_TEXT,
      beforeJson: AUDIT_PRIVACY_REDACTED_JSON, afterJson: AUDIT_PRIVACY_REDACTED_JSON,
    });

    const redactions = await db.select().from(schema.auditEventPrivacyRedactions);
    expect(redactions).toHaveLength(1);
    expect(redactions[0]).toMatchObject({
      auditEventId: event.id, subjectAthleteId: athleteId, redactionVersion: 1,
      redactActorUserId: true, redactSessionId: true, redactReason: true,
      redactBeforeJson: true, redactAfterJson: true, requestedByUserId: 'admin-a',
      maintenanceReference: 'PRIVACY/TICKET-123',
    });

    const auditRows = await db.select().from(schema.auditEvents);
    const original = auditRows.find((row) => row.id === event.id);
    const proofEvent = auditRows.find((row) => row.action === 'audit.privacy_redacted');
    expect(original?.beforeJson).toBe(AUDIT_PRIVACY_REDACTED_JSON);
    expect(proofEvent?.afterJson).toContain('PRIVACY/TICKET-123');
    expect(proofEvent?.afterJson).not.toContain('Petra');

    await expect(db.$client.execute({
      sql: 'UPDATE audit_events SET action = ? WHERE id = ?', args: ['tampered', event.id],
    })).rejects.toThrow('audit events are immutable');
    await expect(db.$client.execute({
      sql: 'UPDATE audit_events SET before_json = ? WHERE id = ?', args: ['{}', event.id],
    })).rejects.toThrow('audit events are immutable');
    await expect(db.$client.execute({
      sql: 'DELETE FROM audit_events WHERE id = ?', args: [event.id],
    })).rejects.toThrow('audit events are immutable');
    await expect(db.$client.execute({
    ...
    await expect(db.$client.execute({
      sql: 'DELETE FROM audit_event_privacy_redactions WHERE audit_event_id = ?', args: [event.id],
    })).rejects.toThrow('audit privacy redactions are immutable');

    await expect(redactHistoricalAuditEventForAthlete(db, 'tenant-a', athleteId, adminActor, {
      auditEventId: event.id,
  ...
  it('rejects rows not selected by the read-only privacy inventory', async () => {
    const db = await createTestDatabase();
    const { athleteId } = await seedReadyAthlete(db, { linkedUserId: null });
    const event = await appendAuditEvent(db, {
      tenantId: 'tenant-a', action: 'tenant.export.created', entityType: 'tenant',
      entityId: 'tenant-a', source: 'WEB', after: { format: 'encrypted' },
    });
    ...
    await expect(redactHistoricalAuditEventForAthlete(db, 'tenant-a', athleteId, adminActor, {
      ...
      assessedAt: '2026-08-05T10:00:00.000Z',
    })).rejects.toThrow('not listed by the athlete privacy inventory');
  ...
  it('fails closed while retention is active and for non-admin actors', async () => {
    const db = await createTestDatabase();
    ...
    const event = await appendAuditEvent(db, {
      tenantId: 'tenant-a', action: 'athlete.updated', entityType: 'athlete',
      entityId: athleteId, source: 'WEB', after: { firstName: 'Petra', athleteId },
    });
