...
describe('athlete anonymization preview', () => {
  it('builds a deterministic tenant-scoped read-only scope without deciding diagnostic deletion', async () => {
    ...
    expect(preview.scopes).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ scope: 'COACH_ASSIGNMENTS', disposition: 'RELATIONSHIP_LINK_REMOVAL_REQUIRED', rowCount: 1 }),
      expect.objectContaining({ scope: 'CONSENT_RECORDS', disposition: 'MINIMIZED_COMPLIANCE_RECORD_REQUIRED', rowCount: 1 }),
      expect.objectContaining({ scope: 'GUARDIAN_RECORDS', disposition: 'THIRD_PARTY_RECORD_REMOVAL_REQUIRED', rowCount: 1 }),
      expect.objectContaining({ scope: 'DELETION_REQUESTS', disposition: 'FREE_TEXT_REDACTION_REQUIRED', rowCount: 1 }),
