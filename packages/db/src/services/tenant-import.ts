...
function requireId(row: PortableRow, path: string): string {
  if (typeof row.id !== 'string' || !row.id) throw new Error(`Missing technical id at ${path}`);
  return row.id;
}

function registerId(map: Map<string, string>, sourceId: string, path: string, override?: string): void {
  if (map.has(sourceId)) throw new Error(`Duplicate technical id ${sourceId} at ${path}`);
...
function remapRow(row: PortableRow, idMap: ReadonlyMap<string, string>, targetTenantId: string): PortableRow {
  return Object.fromEntries(Object.entries(row).map(([key, value]) => {
    ...
    if (typeof value === 'string' && (key === 'id' || key.endsWith('_id'))) return [key, idMap.get(value) ?? value];
    return [key, value];
  }));
}
...
export function buildTenantImportPlan(document: TenantPortabilityExportDocument, targetTenantId: string = crypto.randomUUID(), idOverrides: Readonly<Record<string, string>> = {}): TenantImportPlan {
  ...
  const remappedTables = Object.fromEntries(Object.entries(document.data).map(([tableName, rows]) => [tableName, rows.map((row) => remapRow(row, idMap, targetTenantId))])) as Record<TenantPortabilityTable, PortableRow[]>;
  return {
    sourceTenantId,
    targetTenantId,
...
export async function prepareTenantImportPlan(db: Database, document: TenantPortabilityExportDocument, options: PrepareTenantImportOptions = {}): Promise<TenantImportPlan> {
  const targetTenantId: string = options.targetTenantId ?? crypto.randomUUID();
  const targetTenant = await db.$client.execute({ sql: 'SELECT id FROM tenants WHERE id = ? LIMIT 1', args: [targetTenantId] });
  if (targetTenant.rows.length > 0) throw new Error('Tenant import target id already exists');
  ...
  const slugConflict = await db.$client.execute({ sql: 'SELECT id FROM tenants WHERE slug = ? LIMIT 1', args: [targetSlug] });
  if (slugConflict.rows.length > 0) throw new Error('Tenant import target slug already exists');
  ...
  for (const [index, row] of document.users.entries()) {
    ...
    const existing = await db.$client.execute({ sql: 'SELECT id FROM users WHERE lower(email) = ? LIMIT 1', args: [email] });
  ...
  return {
    ...plan,
    tenant: { ...plan.tenant, slug: targetSlug, ...(options.targetDeploymentMode ? { deployment_mode: options.targetDeploymentMode } : {}) },
    reusedUserIds: Object.freeze([...reusedUserIds]),
  };
}

export async function executePreparedTenantImportPlan(db: Database, plan: TenantImportPlan, options: ExecutePreparedTenantImportOptions = {}): Promise<TenantImportPlan> {
  ...
  try {
    ...
    for (const row of plan.memberships) await transaction.execute(insertStatement('tenant_memberships', row));
    for (const tableName of TENANT_PORTABILITY_TABLES) {
      for (const row of plan.tables[tableName] ?? []) await transaction.execute(insertStatement(tableName, row));
      if (options.failAfterTable === tableName) throw new Error(`Tenant import interrupted after ${tableName}`);
    }
    ...
    await transaction.execute(insertStatement('audit_events', {
      id: crypto.randomUUID(), tenant_id: plan.targetTenantId, occurred_at: now,
      actor_user_id: typeof adminMembership?.user_id === 'string' ? adminMembership.user_id : null,
      actor_role: 'TENANT_ADMIN', action: 'tenant.import.completed', entity_type: 'tenant', entity_id: plan.targetTenantId,
      source: 'TENANT_IMPORT', correlation_id: crypto.randomUUID(), after_json: JSON.stringify({ sourceTenantId: plan.sourceTenantId }),
...
export async function executeTenantImportDatabase(db: Database, document: TenantPortabilityExportDocument, options: ExecuteTenantImportOptions = {}): Promise<TenantImportPlan> {
  const plan = await prepareTenantImportPlan(db, document, options);
  const preparedOptions: ExecutePreparedTenantImportOptions = {};
  if (options.failAfterTable) preparedOptions.failAfterTable = options.failAfterTable;
  return executePreparedTenantImportPlan(db, plan, preparedOptions);
}
