...
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" --profile backup run --rm postgres-backup
...
[[ -f "$backup_dir/$new_bundle.sha256" ]] || { echo "Missing checksum for $new_bundle" >&2; exit 1; }
(
  cd "$backup_dir"
  sha256sum -c "$new_bundle.sha256"
)
...
printf '{"status":"BACKUP_OK","bundle":"%s","retained":%d}\n' "$new_bundle" "$retention"
