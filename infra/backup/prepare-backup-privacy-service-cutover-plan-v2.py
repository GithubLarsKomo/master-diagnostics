#!/usr/bin/env python3
"""Prepare a signed, strictly read-only service cutover plan v2 from TARGET_HANDOFF_VERIFIED."""
...
def validate_rendered_compose(rendered: dict[str, Any]) -> None:
    ...
    if "privacy-capabilities:check" not in command_text:
        fail("SERVICE_CUTOVER_PREFLIGHT_COMMAND_INVALID", "privacy-check service does not run capability checker")
    app = services["app"]
...
def build_record(args: argparse.Namespace, handoff: dict[str, Any], rendered_raw: bytes) -> dict[str, Any]:
    ...
    if not isinstance(activation_id, str) or not ACTIVATION_ID.fullmatch(activation_id):
        fail("ACTIVATION_ID_INVALID", "verified target handoff activation ID is invalid")
    for field in ("planFingerprint", "handoffFingerprint", "handoffFileSha256", "targetConfigAttestationSha256", "targetEnvFingerprint"):
        if not isinstance(handoff.get(field), str) or not SHA256.fullmatch(handoff[field]):
            fail("SERVICE_CUTOVER_HANDOFF_BINDING_INVALID", f"verified handoff {field} is invalid")
    binding = {
