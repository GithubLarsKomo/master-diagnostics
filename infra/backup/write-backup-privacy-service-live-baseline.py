...
def read_existing(path: Path, key: bytes, expected_baseline_id: str, live_state_fingerprint: str, cutover_plan_fingerprint: str) -> dict[str, Any]:
    ...
    if record.get("serviceLiveBaselineVersion") != BASELINE_VERSION or record.get("baselineId") != expected_baseline_id:
        fail("SERVICE_LIVE_BASELINE_CONFLICT", "existing baseline identity is invalid")
    if record.get("liveStateFingerprint") != live_state_fingerprint or record.get("cutoverPlanFingerprint") != cutover_plan_fingerprint:
        fail("SERVICE_LIVE_BASELINE_CONFLICT", "existing baseline is bound to different live state")
...
def main() -> int:
    ...
    try:
        ...
        record = build_record(cutover_record, args.cutover_plan, live_state, captured_at)
        safe_output_root(args.output_root)
        path = args.output_root / f"{record['baselineId']}.json"
        if path.exists():
            envelope = read_existing(
                path,
                key,
                record["baselineId"],
                record["liveStateFingerprint"],
                record["cutoverPlanFingerprint"],
            )
            created = False
            record = envelope["record"]
        else:
