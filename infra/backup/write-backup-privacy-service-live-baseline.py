...
def verify_existing(path: Path, key: bytes, expected_id: str, live_fp: str, cutover_fp: str) -> dict[str, Any]:
    ...
    if record.get("serviceLiveBaselineVersion") != BASELINE_VERSION or record.get("baselineId") != expected_id:
    ...
    if not isinstance(signature, str) or not HMAC_SHA256.fullmatch(signature) or not hmac.compare_digest(signature, sign(record, key)):
        fail("SERVICE_LIVE_BASELINE_CONFLICT", "existing baseline HMAC is invalid")
    return envelope
...
def main() -> int:
    ...
    try:
        ...
        if path.exists():
            envelope = verify_existing(path, key, record["baselineId"], record["liveStateFingerprint"], record["cutoverPlanFingerprint"])
            record = envelope["record"]
            created = False
        else:
