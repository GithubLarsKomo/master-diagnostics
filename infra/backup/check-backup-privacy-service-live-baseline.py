#!/usr/bin/env python3
"""Verify signed service live-baseline evidence and re-attest current Docker state."""
...
def validate_record(record: dict[str, Any], baseline_path: Path, cutover_record: dict[str, Any], cutover_plan: Path) -> dict[str, Any]:
    if record.get("serviceLiveBaselineVersion") != BASELINE_VERSION:
        fail("SERVICE_LIVE_BASELINE_VERSION_INVALID", "baseline version must be 1")
    ...
    expected_cutover = {
        "cutoverId": cutover_record["cutoverId"],
        "cutoverPlanFingerprint": cutover_record["cutoverPlanFingerprint"],
        "cutoverPlanFileSha256": common.sha256_bytes(common.read_file(cutover_plan, "SERVICE_CUTOVER_PLAN", private=True)),
        "activationId": cutover_record["activationId"],
        "renderedComposeSha256": cutover_record["renderedComposeSha256"],
    }
    for field, expected in expected_cutover.items():
        if record.get(field) != expected:
            fail("SERVICE_LIVE_BASELINE_CUTOVER_BINDING_MISMATCH", f"baseline {field} differs from verified cutover plan")
    if record.get("expectedPreCutoverBackupState") != "DISABLED" or record.get("expectedPreCutoverNotificationsState") != "DISABLED":
        fail("SERVICE_LIVE_BASELINE_EXPECTED_STATE_INVALID", "baseline expected pre-cutover privacy state is invalid")
    for field in (
    ...
    for field in ("cutoverMutationStarted", "serviceCutoverExecuted", "liveTargetAttested"):
    ...
    if baseline_id != expected_id:
        fail("SERVICE_LIVE_BASELINE_ID_MISMATCH", "baseline ID does not match signed state identity")
...
def main() -> int:
    ...
    try:
        ...
        verified = common.verify_cutover_plan(
            args.cutover_plan_checker,
            completion_checker=args.completion_checker,
        ...
        cutover_record = common.read_cutover_record(args.cutover_plan, verified)
        rendered = common.render_compose(args.compose_file, args.env_file, cutover_record["renderedComposeSha256"])
        ...
        signed_state = validate_record(record, args.baseline, cutover_record, args.cutover_plan)
