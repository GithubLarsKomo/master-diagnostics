#!/usr/bin/env python3
"""Read-only helpers for backup-privacy service live-baseline evidence."""
...
def run_json(command: list[str], code: str) -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        ...
        fail(f"{code}_FAILED", detail or f"command exited {proc.returncode}")
    try:
        payload = json.loads(proc.stdout.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"{code}_OUTPUT_INVALID: command did not return JSON") from exc
    ...
    *,
    ...
    proc = subprocess.run(
        [
            ...
            "--plan-checker",
            str(activation_plan_checker),
            "--evidence-checker",
            str(execution_evidence_checker),
            "--activation-plan",
            str(activation_plan),
            "--pending",
            str(pending),
    ...
    if proc.returncode != 0 or result.get("status") != "SERVICE_CUTOVER_PLAN_VERIFIED" or result.get("serviceCutoverExecutionAllowed") is not True:
...
def read_cutover_record(path: Path, verified: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(record, dict) or record.get("serviceCutoverPlanVersion") != 1:
        fail("SERVICE_CUTOVER_PLAN_INVALID", "cutover-plan record is invalid")
