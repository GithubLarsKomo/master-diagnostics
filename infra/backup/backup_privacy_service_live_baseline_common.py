...
def read_cutover_record(path: Path, verified: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in ("cutoverId", "activationId", "cutoverPlanFingerprint", "renderedComposeSha256"):
    ...
    if record.get("authorizationSource") != "TARGET_HANDOFF_VERIFIED":
        fail("SERVICE_CUTOVER_PLAN_AUTHORIZATION_INVALID", "cutover plan is not handoff-authorized")
    for field in ("liveBaselineRequiredBeforeMutation", "targetHandoffRequiredBeforePlanning"):
...
def resolve_container_id(project: str, service: str) -> str:
    proc = subprocess.run(
        ["docker", "ps", "-a", "--no-trunc", "--filter", f"label=com.docker.compose.project={project}", "--filter", f"label=com.docker.compose.service={service}", "--format", "{{.ID}}"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def state_snapshot(container: dict[str, Any], service: str) -> dict[str, Any]:
    ...
    return {"running": True, "status": "running", "healthStatus": health_status, "startedAt": started_at, "restartCount": restart_count}
...
def caddy_volumes(rendered: dict[str, Any], caddy: dict[str, Any]) -> dict[str, str]:
    ...
    for role, target in CADDY_VOLUME_TARGETS.items():
        compose_matches = [item for item in compose_volumes if isinstance(item, dict) and item.get("type") == "volume" and item.get("target") == target and isinstance(item.get("source"), str)]
        ...
        mount_matches = [item for item in mounts if isinstance(item, dict) and item.get("Type") == "volume" and item.get("Destination") == target and isinstance(item.get("Name"), str)]
