...
Seit #238 bleibt der nackte Cutover-Plan zusätzlich strikt pre-baseline und liefert `serviceCutoverExecutionAllowed=false`. Erst die unabhängige Verifikation der hier beschriebenen Live-Baseline darf das nächste Mutations-Gate öffnen.
...
Damit bleiben Target-Handoff, Target-Environment, Target-Konfigurations-Attestation und Compose-Render Teil jeder Baseline-Prüfung. Das `false` des Plan-Checkers ist kein Fehler, sondern die explizite Sicherheitsgrenze vor einer unabhängig verifizierten Live-Baseline.
...
Der Writer selbst liefert weiterhin `serviceCutoverExecutionAllowed=false`, weil das persistierte Artefakt erst noch unabhängig gegen den aktuellen Docker-Istzustand re-attestiert werden muss.
...
Der Contract baut zunächst die vollständige kanonische #234/#238-Kette bis zum weiterhin gesperrten Service-Cutover-Plan v2 auf und erzeugt danach echte kurzlebige Docker-Fixtures für den Pre-Cutover-Istzustand.

Geprüft werden:

- der Plan-Checker bleibt vor der Baseline bei `serviceCutoverExecutionAllowed=false`,
...
- unabhängige Live-Re-Attestation öffnet erst danach `serviceCutoverExecutionAllowed=true`,
...
Die Legacy-v1-Baseline aus #237 bleibt absichtlich fail-closed und kann das #238-Gate nicht umgehen.
